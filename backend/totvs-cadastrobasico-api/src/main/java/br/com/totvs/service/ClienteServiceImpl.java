...
public class ClienteServiceImpl implements ClienteService {
	 ...
	 *
	 * Este método é responsável por enviar uma solicitação POST para adicionar um novo cliente ao servidor JSON.
	 * O cabeçalho da solicitação é configurado para indicar que o conteúdo é do tipo JSON.
	 *
	 * @param request ClienteRequest - O objeto de solicitação contendo informações do cliente a ser adicionado.
	 * @return Cliente - O cliente adicionado, conforme retornado pelo servidor JSON.
	 * @see ClienteRequest
	 */
	@Override
	 ...
	 *
	 * Este método realiza uma solicitação GET ao servidor JSON para obter a lista de clientes.
	 * O cabeçalho da solicitação é configurado para aceitar dados no formato JSON.
	 *
